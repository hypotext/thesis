...
First, take any family of PRFs $$F_n = \{f_k : \{0,1\}^n \rightarrow \{0,1\}^n\}_{k \in \{0,1\}^m}.$$

Construct a PRG $$H : (x : \mathbb{N}) \rightarrow \{0,1\}^n \rightarrow \{0,1\}^{xn}$$
...
\item[] else, compute $f_k(v)$, and return\\ $f_k(v) || H(f_k(v), n-1).$
\end{itemize}

The user (or adversary) can request any polynomial number of blocks $x$. Thus, the stretch of $H$ is $l(n, x) = nx$.
...
\textbf{Definition of indistinguishability.} For all non-uniform, non-adaptive probabilistic polynomial time distinguishers $D$, polynomial number of blocks $x$, there exists a negligible function such that
$$| \Pr [ D (H (x, U_n) = 1] - \Pr [ D(U_{nx}) = 1] | \leq neg(n),$$

where $U_n \xleftarrow{R} \{0,1\}^n$ ($U_n$ is sampled uniformly at random from $\{0,1\}^n$), $U_{nx} \xleftarrow{R} \{0,1\}^{nx}$, and $k \xleftarrow{R} \{0,1\}^n$ ($k$ is the key for the PRF).
...
\begin{itemize}
\item[] $n \leftarrow D$ (the distinguisher picks a number of blocks)
\item[] $bits \leftarrow \{0,1\}^{nx}$
\item[] $ret \s D(bits)$
\end{itemize}

The difference between these games is...

How this relates to the indistinguishability definition... (one-half)

Concrete bound

Intermediate game

Bounding the difference between the real-world game and the intermediate game

PRF advantage games

Bounding the difference between the intermediate game and the ideal-world game
