...
We pick a bad event, show that the two games are identical until bad, and bound the probability of the bad event of the bad event happening.

The bad event is much the same as before, in Lemma REFERENCE. Intuitively, we need to ``zoom in'' on the inputs and outputs to the oracles in only the $i+1$th call of each game.

\begin{dfn}The \textbf{bad event} is that there are duplicates in the inputs to the oracle used on the $i+1$th call.\end{dfn} 

Expose the bad event in both games.

State the theorem in terms of first and second.

\begin{lem}The two games (name them TODO) are identical until bad. \end{lem}
\begin{proof} 
We prove the two assumptions.
...
\begin{lem}The probability of the bad event is $\f{(l_i+1)^2}{2^c}$. (TODO: probability of that game returning true) \end{lem}
\begin{proof} 
Use oracle eq until bad? 

Then reuse collision probability proof. 
\end{proof}

This completes bound on the distance between hybrid $i+1$ and the intermediate hybrid for $i$.
\end{proof}
