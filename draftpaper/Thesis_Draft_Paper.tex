...
PRF adversary. Why do we need this? To go from PRF to RF
...
Gi_prf_rf_close_i : forall (i : nat),
  ...
  \end{lstlisting}
   ...
   Gi_rf 2:   RB RB   RF PRF
	...
	The difference in probability that a new adversary, the PRF adversary, realizes this has happened is defined by cryptographers to be \lstinline|PRF_Advantage|. 
...
\end{lstlisting}
	...
	Questionable: this difference in probability (\lstinline|PRF_Advantage|) is defined the same no matter how many times the oracle in question is called within \li|GenUpdate| (and it is called $2 + numBlocks$ times, not just once).
...
There are 21 admitted lemmas left to prove that I'm confident about, and 1 lemma left that I'm sure is true, but am not sure how to prove (\lstinline|hi|).
