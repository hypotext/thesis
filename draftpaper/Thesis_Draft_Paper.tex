...
TODO: $compMap$ here and in the game below \\
...
In the ideal-world game, $GenRand$ takes the list $l$ outputted by the adversary and generates $n$ blocks that is each sampled uniformly at random.\\
...
% In our case, we simplify the proof by making an assumption that implies that the probabilities of distinguishing between adjacent hybrids are all the same:
% \begin{gather*}
% \forall i, j \in [0, t-1],\\ Adv_{H_i, H_{i+1}}(A) = Adv_{H_j, H_{j+1}}(A). 
% \end{gather*}
% Namely, we make the simplifying assumption that all elements are the same in the list $l$ of blocks requested by the adversary. The element in question could be anything; in our proof, we set it to the maximum number of blocks requestable.\footnote{This is not a realistic assumption in practice. Also, one can contrive a PRG that is insecure for certain hardcoded block request numbers. We plan to extend our pseudorandomness proof to arbitrary lists of block numbers chosen by the adversary. 

% Proof sketch: Let $l_i$ be the number of blocks requested by the adversary on the $i$th call to $GenUpdate$. Then $$Adv_{H_i, H_{i+1}}(A) \leq PRF\_Advantage + Pr\_collisions(l_i)$$ where $Pr\_collisions(n) = \f{(n+1)^2}{2^c}.$ This is negligible for bounded $n$ and sufficiently large $c$.}

% Using this assumption, 
% \begin{gather*}
% \forall i \in [0, t-1],\\ Adv_{D_1, D_2}(A) \leq t \cdot Adv_{H_i, H_{i+1}}(A),
% \end{gather*}

% so we need only bound $Adv_{H_i, H_{i+1}}(A)$.
...
Let's define a notation for the $i$th hybrid and write out all the hybrids for some $i$. Take $i = 3$ (so the adversary makes $3$ calls to $GenUpdate$ with some number of blocks requested). Then there are four hybrids.
...
In place of the PRF, $H_i$ uses ideal randomness for calls $\leq i$. So its output is ideally random for calls $\leq i$.

In place of the PRF, $H_{i+1}$ uses ideal randomness for calls $\leq i + 1$. So its output is ideally random for calls $\leq i+1$.

We introduce an intermediate hybrid $H_i'$ between them. For calls $\leq i$, just like $H_i$, $H_i'$ uses ideal randomness in place of the PRF. On call $i$, $H_i$ uses a random function in place of the PRF. For calls $> i$, $H_i$ uses the PRF normally.

% TODO: difference between hybrids and games?
Phrase each in terms of games

Actually, the $G\_real$ and $G\_ideal$ above are wrong...

We introduce an intermediate game $Gi\_rf$ between the two corresponding games TODO.

As before, we bound the distance between via the triangle inequality. This is very similar to our proof in Theorem REFERENCE. Reduces to Adam's because of the hybrid ordering (reduces to $Instantiate$)

Intuition: 

\begin{ex} \label{ex:hybrid_intermediate} Continuing our hybrid example from earlier (Example \ref{ex:hybrid_notation}),  
\end{ex}

\begin{lem}$$Adv_{H_i, H_i'}(A) \leq PRF\_Advantage \s i.$$ \end{lem}
\begin{proof} 
Phrase in terms of $PRF\_Advantage$ and $PRF\_A$. Similar to Lemma REFERENCE, but does not reduce to it.

TODO: there are multiple PRF adversaries, so we need an upper bound 

TODO: we move the $v$ because we re-key the PRF, so it'd hard to phrase it in terms of $PRF\_A i$? It would be really messy (and maybe impossible). $PRF\_A i$ is given $f_k$ oracle or RF oracle. The oracle query has to happen across two adjacent $GenUpdate$ calls ($v\_update$ from previous call if it exists, then in current call the $Gen\_loop$ and PRF re-keying). Write out the iterations sideways

TODO: was there a second reason for this?

TODO: same PRF in $Gen\_loop$

% subsubsections are confusing. rephrase this as lemma TODO
\subsubsection{Updating the v}
...
This version of $GenUpdate\_original$ omits the last line, which updates $v$. \\
...
This version of $GenUpdate\_original$ starts by updating $v$ and does not update $v$ again. \\
...
Reuse identical until bad proof. 

Reuse collision probability proof.
\end{proof}
...
This completes our bound on the distance between adjacent hybrids.
...
TODO: Some words on dealing with the hybrids formally

Direction of proof we could have taken

No v-updating (see Matt's email on Dec 22)
