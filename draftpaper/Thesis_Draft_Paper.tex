...
Construct a PRG $$H : (n : \mathbb{N}) \rightarrow \{0,1\}^c \rightarrow \{0,1\}^{cn}$$

which takes an input and applies the PRF to it, then uses the output as one block of pseudorandom bits and as the next input to the PRF.  \\

% TODO fix formatting
$H(n, v) :=$
\begin{game}
\item[] if $n = 0$ then return $v$
\item[] else return $f_k(v) || H(f_k(v), n-1).$\\
\end{game}

The user (or adversary) can request any polynomial number of blocks $n$, and the length of the seed is $c$. Thus, the stretch of $H$ is $l(n, c) = cn$. Let's take $k \xleftarrow{R} \bt^c$ to be a parameter accessible by $H$.
...
$$| \Pr [ D (H (n, U_c) = 1] - \Pr [ D(U_{cn}) = 1] | \leq neg(c,n),$$
...
The probability that the distinguisher returns $1$ when given the PRG's output, $\Pr[D(H(n, U_c) = 1]$, is the same as the probability that the result of this real-world game is $1$. \\

$\textbf{G\_real} := $
\begin{game}
\item[] $n \leftarrow D$ (the distinguisher picks a number of blocks)
\item[] $v \xleftarrow{R} \{0,1\}^c$
\item[] $bits \leftarrow H(n,v)$
\item[] $ret \s D(bits)$ \\
\end{game}

That is, $\Pr[D(H(n, U_c) = 1)] = \Pr[G\_real]$

The probability that the distinguisher returns $1$ when given a uniformly randomly sampled bit string of the right length is the same as the probability that the result of this ideal-world game is $1$.\\

$\textbf{G\_ideal} := $
\begin{game}
\item[] $n \leftarrow D$ (the distinguisher picks a number of blocks)
\item[] $v \xleftarrow{R} \{0,1\}^c$
\item[] $bits \leftarrow H\_oracle(n,v)$
\item[] $ret \s D(bits)$ \\
\end{game}
...
\begin{game}
\item[] if $n = 0$ then return $v$
\item[] else return $r(v) || H(r(v), n-1).$\\
\end{game}
...
\begin{thm} $| \Pr[G\_real] - \Pr[G\_ideal]| = $\\
$|\Pr[G\_real] - \Pr[G\_intermediate]|$ \\
$-  |\Pr[G\_intermediate] - \Pr[G\_ideal]|.$ \end{thm}
\begin{proof} By the triangle inequality. \end{proof}

\begin{thm} $|\Pr[G\_real] - \Pr[G\_intermediate]| \leq PRF\_Advantage.$ \end{thm}
\begin{proof} 
...
Since we are doing proofs of concrete security, we must use this definition of the difference between a PRF and a RF. Conventionally, one would reduce the security of the PRF to the security of the PRF, but that would not yield a concrete bound. 
...
In $G\_real\_2$, the key sampling is omitted because we have already sampled it in a global context and made it accessible to the PRG, for convenience.\\
...
where $PRF\_A$, given either the PRF oracle or the RF oracle, simply generates the pseudorandom bits using the oracle and calls the existing PRG adversary on them.\\
...
\begin{game}
\item[] if $n = 0$ then return $v$
\item[] else return $o.query(v) || H(o.query(v), n-1).$\\
\end{game}
...
\begin{lem} $\Pr[G\_intermediate] = \Pr[G\_intermediate\_2].$ \end{lem}
\begin{proof} By substitution and program equivalence.  \end{proof}

Thus, the theorem follows:
$$|\Pr[G\_real] - \Pr[G\_intermediate]| \leq PRF\_Advantage.$$ \end{proof}

Next, we bound the difference between the intermediate and the ideal-world game. This requires doing actual math, not just reasoning about definitions.
...
Instead of using $G\_intermediate$, we modify $G\_intermediate\_2$, which we had proven equivalent to $G\_intermediate$ in Lemma 3.2. We replace the random function oracle with a random function oracle $rf$ that keeps track of all (input, output) pairs, including duplicate inputs. \\
...
The three games are equivalent.

\begin{lem} $$\Pr[G\_intermediate\_2] = \Pr[G\_intermediate\_3]$$ $= \Pr[G\_intermediate\_4].$\end{lem}
...
\begin{lem}$$\Pr[G\_ideal] = \Pr[G\_ideal\_2] = \Pr[G\_ideal\_3].$$\end{lem}
\begin{proof}By unfolding the definition of $PRF\_A$ and program equivalence. Also, adding state and duplicate tracking to the oracle does not change its outputs, and outputting whether the bad event happened does not change the adversary's guess. \end{proof}
...
\begin{gather*}
|\Pr[G\_intermediate\_4] - \Pr[G\_ideal\_3]| \leq \\ \Pr[G\_ideal\_3 \s sets \s bad].
\end{gather*}
This is convenient, because now we can just work with a single game, which allows us to apply our game-equivalence techniques. Now we only need to bound $\Pr[G\_ideal\_3 \s sets \s bad]$.

More formally, we define the probability that a game $G$ sets bad as $\Pr[G\_B]$, where $G\_B$ is a version of $G$ that only returns whether the bad event occurred. Here, for example:\\
...
Now we bound the probability of the bad event happening.

\begin{lem} $$\Pr[G\_ideal\_3B] \leq \f{n^2}{2^c}$$ \end{lem}
\begin{proof} 
...
To be more precise, $G\_ideal\_3B$ generates a list of $n-1$ such bitvectors (inputs) with a randomly-sampled seed $v$ at the beginning, which is the first input.\\

$\textbf{G\_ideal\_4} := $
\begin{game}
\item[] $bits \leftarrow generate\_n\_bv\_of\_len\_c$
\item[] $ret \s (hasDups(bits))$ 
\end{game}

\begin{lem}$\Pr[G\_ideal\_3] = \Pr[G\_ideal\_4]$.\end{lem}
\begin{proof}Program equivalence and the irrelevance of the adversary.\end{proof}

Thus, we need only upper-bound the probability that there is a duplicate in a list of $n$ uniformly-sampled bitvectors of length $c$. 
...
This completes our bound on the probability of the bad event.
\end{proof}
...
Thus, $H$ is a secure PRG. Moreover, since we did our proof in the concrete security model, we have a better idea of how secure it is (modulo the black box of $PRF\_Advantage$).
