...
Thus, the endpoint hybrids correspond correctly to the quantity defined in \ref{def:indist}:
...
In $H_1$, the first call to $GenUpdate$ generates bits in $Gen\_loop$ uniformly sampling from $\{0,1\}^c$ instead of calling $f_k$. Subsequent calls use the normal $Gen\_loop$ construction, which calls $f_k$. Hence we denote $H_1$ as \\ \li|RB PRF PRF|.
...
Note that there is state being kept between calls and updated in calls. $(k,v)$ is instantiated, then passed into call 1 of $GenUpdate$, updated there and passed into call 2 of $GenUpdate$, and so on for the rest of the calls.

Intuitively, we replace PRFs with RBs from left to right. The order matters. In $H_0$, $(k,v)$ are passed in from $Instantiate$, which uniformly randomly samples them. In $H_i$, $(k,v)$ are passed in to the call $i+1$ (the first call with the PRF) from call $i$ (the last call using RB), which uniformly randomly samples $k$ and $v$ when they are updated. That makes it easier to reason about subsequent PRF-using calls, which we will have to do later.
...
Again, we are seeking to bound
$$Adv_{D_1, D_2}(A) \leq \sum_{0}^{t-1} Adv_{H_i, H_{i+1}}(A).$$

Now we bound the quantity in the summation. Intuitively, we have just reduced it to our earlier proof about the security of $H$, and the bound is the same.
...
\begin{gather*}
Adv_{H_i, H_{i+1}}(A) \leq PRF\_Advantage + \f{(l_i+1)^2}{2^c}.
\end{gather*} \end{thm}
\begin{proof} 
...
Reusing Adam's work. Relation to Adam's proof: we reduce it to his proof in the cases of
\begin{itemize}
...
We prove that the pseudorandom output produced by this sequence of calls $$[GenUpdate\_original, GenUpdate\_original, \ldots]$$ is identically distributed to the pseudorandom output produced by this new sequence of calls $$[GenUpdate\_noV, GenUpdate, GenUpdate, \ldots].$$ 
...
\begin{gather*}
Adv_{D_1, D_2}(A) \leq t \cdot PRF\_Advantage + \sum_{0}^{t-1} \f{(l_i+1)^2}{2^c} \\ 
\leq t \cdot (PRF\_Advantage + \f{(maxBlocks + 1)^2}{2^c}).
\end{gather*}
\end{thm}
\begin{proof}
