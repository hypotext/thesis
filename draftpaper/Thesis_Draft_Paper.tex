...
\title{Formal verification of the HMAC-DRBG pseudorandom number generator}
...
$oracle\_i\_useO$ ($^*Oi\_oc'$) replaces the oracle used in call $i+1$ with the provided oracle, and behaves like $oracle\_i$ otherwise (replaces the PRF with ideal randomness for calls $\leq i$, and uses the PRF normally for calls $> i+1$). 
...
Now we rewrite $G\_hybrid(i)$ and $G\_hybrid\_rf(i)$ in terms of $PRF\_Advantage$ and $PRF\_A$. 

Similar to Lemma REFERENCE, but does not reduce to it.

There are multiple PRF adversaries, so we take the final bound to be the greatest of the $PRF\_Advantage$s.

This completes our proof of $PRF\_Advantage\_Max$.
\end{proof}

\begin{lem}$$|\Pr[G\_hybrid\_rf(i)] - \Pr[G\_hybrid\_(i+1)] | \leq \f{(l_i + 1)^2}{2^c}.$$ \end{lem}
\begin{proof} 
...
There are 21 admitted lemmas left to prove that I'm confident about.

Matt and I have started working on the backtracking resistance proof. It relies heavily on reusing our work on the indistinguishability proof, modulo adaptiveness of adversary. So, we will concurrently work on writing an informal proof of that and formalizing the specifications and definitions in FCF. 

Also, we need to figure out how to adapt the indistinguishability proof to a nonadaptive adversary that picks any list of $blocksPerCall$ and $numCalls$ beforehand, not the hardcoded $maxBlocksPerCall$ and $maxCalls$.

I also need to do the spec equivalence proof.
...
We have contributed a paper proof of the pseudorandomness of a simplified version of HMAC-DRBG. People had already written paper proofs, but they were complicated and not linked to functional specifications or implementations.
...
We have contributed a paper proof that a simplified version of HMAC-DRBG satisfies backtracking resistance. No such proof existed before.

We have contributed knowledge about the challenges of formally verifying PRGs, and especially about proving things about hybrid arguments.
...
I also want to thank Matt and Andrew for suggesting a thesis topic that was both cool and well-motivated.

Lennart Beringer came to all of my thesis meetings and helped debug my proofs on the weekend.

Everyone above provided valuable feedback on drafts of this thesis.

Naphat Sanguansin did the work that would allow my proofs to actually hold on an implementation of HMAC-DRBG.

% Naphat Sanguansin 

Thanks to everyone in 2D for general encouragement, cooking great food that sustained me at late hours, and for dealing with me camping out in the dining room (hi, Jaehwan).
