...
%@ cite and add names
There exist only two prior proofs of security of HMAC-DRBG. The first, by Champagna, assumes that HMAC is a random oracle, which is a very strong assumption. Thus, cryptographers generally don't trust proofs written in this model. \ct{@cite, add specific quote} Additionally, this paper is not peer-reviewed. The second, by Hirose, was peer-reviewed, but its proof is lengthy and involved. Neither paper's proofs has been machine-checked or linked to an implementation of HMAC-DRBG.
...
\end{lstlisting}
  ...
  TODO: Talk about the 8 reductions/equivalences in \li|Gi_rb_bad_collisions|
...
Concretely, this is true for \li|Gi_rb_bad| and \li|Gi_rf_bad| because if there are no duplicates in the $i$th oracle call's inputs, clearly the random function behaves exactly like uniformly sampling random bitvectors. So their output should be indistinguishable (or identical, if you provide each run with the same "coins" of randomness). % TODO is this right? how does PRHL / postcondition work?

To prove these two, I push both of these statements up into a postcondition. I combine them into the following postcondition:
...
Basically, after the two \li|PRF_Adversary| runs (one with the RB oracle, one with the RF oracle), the following relation holds on their outputs:

\begin{itemize}
\item There is a duplicate in one oracle's state iff there is a duplicate in the other oracle's state, and 
\item if there are no duplicates in either of their states, then their states are identical and their adversaries return the same guess. This is intuitively true because the output bits (given to the adversary) are the same in both cases.
\end{itemize}

Pushing this back further into the oracle computations, I have this postcondition specification on the \li|oracleCompMap_inner| computation.
...
This says that the following postcondition holds on the result of running \li|oracleCompMap_inner| with the RB oracle or the RF oracle respectively:

\begin{itemize}
\item there are duplicates (as defined by \li|dupsInIthCallInputs_only|) in the inputs to the RB oracle iff there are duplicates in the inputs to the RF oracle. (TODO: but only on the ith call?)
\item if there are no duplicates in the inputs to either oracle, then the output bits are the same and the states are the same between the two oracles.
\end{itemize}

To prove it, I have to use an oracle version of identical until bad, \li|fcf_oracle_eq_until_bad| applied with the postcondition (TODO). This might allow me to skip over oracle computations and just prove things about the oracle? Adam has already done this, though?

hasDups? randomFunc that keeps track of all inputs/outputs?
...
\bibitem{namedate}
(TODO: add citations.) Last, First, Middle. \emph{Title.} Source
